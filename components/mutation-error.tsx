...
export function MutationError({ message, onDismiss }: MutationErrorProps) {
  return (
    <View className="mt-4 flex-row items-center gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3">
      <Text
        accessibilityLiveRegion="assertive"
        accessibilityRole="alert"
        className="min-w-0 flex-1 text-sm font-semibold leading-5 text-risk"
      >
        {message}
      </Text>
      <Pressable
